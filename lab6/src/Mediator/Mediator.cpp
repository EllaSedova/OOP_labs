...
void Mediator::createFirstLevel() {
    ...
    FieldGenerator<DefaultRule<6,22,2>,EventRule<1, 10>,EventRule<2, 9>, //EventRule 0 - wall, 1 - fire, 2 - heal
            EventRule<0, 20>,DamageRule<50>,HealRule<25>,DefaultRule<5,0,0>,
            DefaultRule<4,1,1>,FieldSizeRule<15,4>> gen;
    field = gen.generate();
...
void Mediator::saveGame() {
    player->savePlayer("SavePlayer.txt");
    field->saveField("SaveField.txt");
    std::cout<<"Save game!"<<'\n';
}

void Mediator::reloadGame() {
    try {
        player->reloadPlayer("SavePlayer.txt");
    }
    catch (const char* msg) {
            std::cerr << msg << std::endl;
        std::cerr<<player->gethashPlayer()<<" not equal "<< player->gethashPlayerfile()<< std::endl;
        }
    try {
        field->reloadField("SaveField.txt");
    }
    catch (const char* msg) {
        std::cerr << msg << std::endl;
        std::cerr<<field->gethashField()<<" not equal "<< field->gethashFieldfile()<< std::endl;
    }
}
...
//сделать так, чтобы нормально работал printField() после выгрузки сохранения